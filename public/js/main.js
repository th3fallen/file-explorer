...
export const buildPath = (name, parentPath) => {
...
const createFolderIcon = (isExpanded = false) => {
...
const createIcon = (type, isExpanded = false) => {
...
// State management
const createInitialState = (treeView, listView) => ({
...
// UI update functions
const createListRow = (icon, text, modified, size, onClick) => {
    const row = createElement('tr', { className: 'list-item' });
    const nameCell = createElement('td');
    nameCell.append(icon, text);
    row.append(nameCell, createElement('td', { textContent: modified || '--' }), createElement('td', { textContent: size || '--' }));
    if (onClick) {
        row.addEventListener('click', onClick);
    }
    return row;
};
const updateListView = (state, data) => {
    const fragment = document.createDocumentFragment();
    if (state.currentPath !== '/') {
        const parentIcon = createFolderIcon();
        const parentRow = createListRow(parentIcon, '...', undefined, undefined, () => handleFolderSelect(state, getParentPath(state.currentPath), 'list'));
        fragment.appendChild(parentRow);
    }
    data.forEach(item => {
        const icon = createIcon(item.type, item.type === 'folder' && state.expandedFolders.has(buildPath(item.name, state.currentPath)));
        const row = createListRow(icon, item.name, new Date(item.modified).toLocaleString(), item.type === 'file' ? formatSize(item.size || 0) : '--', item.type === 'folder'
...
const updateTreeView = (state, data, parentElement = state.treeView, level = 0, parentPath = '') => {
    if (!parentElement)
        return;
    const fragment = document.createDocumentFragment();
    if (level === 0) {
...
const updateListViewExpansion = (state) => {
...
// Event handlers
const removeExpandedFolder = (state, path) => {
...
const toggleFolderExpansion = async (state, path, forceState) => {
    if (path === '/')
        return;
...
const handleFolderSelect = async (state, path, source) => {
...
const expandTreeToPath = async (state, path) => {
...
const updateSelection = (state, path) => {
...
// Initialize
export const createFileExplorer = (treeViewId, listViewId) => {
